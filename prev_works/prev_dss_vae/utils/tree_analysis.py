...
from prev_works.prev_dss_vae import GlobalNames
from prev_works.prev_dss_vae import FScore
from prev_works.prev_dss_vae import PhraseTree
from prev_works.prev_dss_vae.utils.utility import write_docs
from prev_works.prev_dss_vae.tools import add_backtrack
from prev_works.prev_dss_vae.tools import tree_sequence_check
from prev_works.prev_dss_vae.tools import tree_sequence_fix
from prev_works.prev_dss_vae.tools import sequence_to_tree
...
def eval_s2t(preds, golds):
    ...
    for pred, gold in zip(preds, golds):
        if tree_sequence_check(pred):
            eval_gold.append(sequence_to_tree(s2t_str=gold))
            eval_pred.append(sequence_to_tree(s2t_str=pred))
        else:
            eval_gold.append(sequence_to_tree(s2t_str=gold))
...
def eval_s2t_robust(preds, golds):
    ...
    eval_pred = []
    for pred, gold in zip(preds, golds):
        pred, error_fix = tree_sequence_fix(pred, fm=GlobalNames.get_fm())
        error_sum_fix += error_fix
        if error_fix > 0:
            error_fix_sents += 1
        gold, _ = tree_sequence_fix(gold, fm=GlobalNames.get_fm())
        if tree_sequence_check(pred):
            eval_gold.append(sequence_to_tree(s2t_str=gold))
            eval_pred.append(sequence_to_tree(s2t_str=pred))
        else:
...
def eval_s2b(preds, golds):
    ...
    eval_pred = []
    for pred, gold in zip(preds, golds):
        pred, error_fix = add_backtrack(pred, fm=GlobalNames.get_fm())
        error_sum_fix += error_fix
        if error_fix > 0:
            error_fix_sents += 1
        gold, _ = add_backtrack(gold, fm=GlobalNames.get_fm())
        if tree_sequence_check(pred):
            eval_gold.append(sequence_to_tree(s2t_str=gold))
            eval_pred.append(sequence_to_tree(s2t_str=pred))
        else:
