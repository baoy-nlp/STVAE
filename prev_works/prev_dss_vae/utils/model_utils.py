...
from prev_works.prev_dss_vae import init_create_model
from prev_works.prev_dss_vae import load_static_model
from prev_works.prev_dss_vae import Dataset
from prev_works.prev_dss_vae import Vocab
...
def update_lr(args, opt, train_iter):
    if args.lr_schedule == "fixed":
        ...
        opt.param_groups[0]['lr'] = get_lr_anneal(args, train_iter + 1)
    elif args.lr_schedule == "transformer":
        opt.param_groups[0]['lr'] = lr_transformer(args, train_iter + 1)
    return opt
...
def prepare_experiment_dir(main_args, model_args, check_dir=True):
    if main_args.exp_name is not None:
        dir_path = [model_args.model_select, main_args.exp_name]
    else:
        dir_path = [model_args.model_select]
    model_name = ".".join(dir_path)
    task_dir = os.path.join(main_args.model_dir, model_args.task_type)
...
def load_model(main_args, model_args, check_dir=True, use_cuda=None):
    ...
    dir_ret = prepare_experiment_dir(main_args=main_args, model_args=model_args, check_dir=check_dir)
