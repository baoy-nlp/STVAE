...
from discriminator.data import NLIDataset
from discriminator.helper import train, validate
from discriminator.model import ESIM
...
def main(train_file,
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        print("-> Training time: {:.4f}s, loss = {:.4f}, accuracy: {:.4f}%".format(
        ...
        torch.save(
            {"epoch": epoch,
             ...
             "valid_losses": valid_losses
                ...
                target_dir, "esim_{}.pth.tar".format(epoch)
            )
        )
...
def train_quora():
    default_config = "../configs/run/quora_training.json"

    parser = argparse.ArgumentParser(description="Train the ESIM model on Quora")
    ...
    main(os.path.normpath(os.path.join(script_dir, config["train_data"])),
         ...
         os.path.normpath(os.path.join(script_dir, config["embeddings"])) if config['embeddings'] is not None else None,
         ...
         args.checkpoint,
         vocab_file=os.path.normpath(os.path.join(script_dir, config["vocab_file"])),
