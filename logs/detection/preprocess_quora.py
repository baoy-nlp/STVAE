...
from discriminator.helper import PLProcessor as Preprocessor


def preprocess_Quora_data(
        inputdir,
        embeddings_file,
        targetdir,
        lowercase=False,
    ...
    for file in os.listdir(inputdir):
        if fnmatch.fnmatch(file, "*train.txt"):
            train_file = file
        elif fnmatch.fnmatch(file, "*valid.txt"):
            dev_file = file
        elif fnmatch.fnmatch(file, "*test.txt"):
            test_file = file
...
def preprocess_quora_main():
    default_config = "../configs/data/Quora-Detection-40K.json"
