from dss_vae.modules.latent import SyntaxGMMLatentConstructor
from .syntax_vae import SyntaxVAE


class SyntaxGMMVAE(SyntaxVAE):
    def __init__(self, args, vocab, embed=None, name='Syntax-GMM-VAE'):
        super(SyntaxGMMVAE, self).__init__(args, vocab, embed, name)
        self.latent_constructor: SyntaxGMMLatentConstructor = SyntaxGMMLatentConstructor(args)
    ...
    def forward(self, input_var, feed_var=None, forcing_ratio=1.0):
        ...
        input_ret = self.latent_constructor(input_ret, is_sampling=self.training, temperature=self.gumbel_temp,
