...
class Template(GaussianLatent):
    ...
    def reconstruct(self, z):
        return self.prior_net.forward_x(z)

    def posterior(self, x1, x2=None, temperature=1.0, hard=False):
        assert x2 is not None, "s can not be the None"
        x1 = x1.reshape(x1.size(0), -1)

        posterior_ret = self.posterior_net(x1, x2, temperature, hard)
    ...
    def prior(self, batch_size):
        c = self.prior_net.forward_c(batch_size)  # uniform prior
        d = self.prior_net.forward_d(batch_size)  # gaussian prior
    ...
    def sampling(self, pos_ret, mode="prior", beam_size=1):
        """
        :param pos_ret: batch_size, mean, var, s, z, c
        :param mode:
        :param beam_size:
        :return:
        """
        ...
        if mode == "prior":
            c = self.prior_net.forward_c(batch_size * beam_size)
            d = self.prior_net.forward_d(batch_size * beam_size)
        elif mode == "map":
            ...
            d = self.posterior_net.infer_d(s=s, c=c)[0]
        ...
        z = self.prior_net.forward_z(d=d, c=c)
        return z
