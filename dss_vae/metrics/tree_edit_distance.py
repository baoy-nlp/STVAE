...
def compute_ted(inputs_1, inputs_2):
    distance_list = []
    for i1, i2 in zip(inputs_1, inputs_2):
        distance_list.append(zss.simple_distance(i1, i2, WeirdNode.get_children, WeirdNode.get_label, weird_dist))
    return distance_list


def evaluate_transfer_distance(ori, tgt, pred):
    src_list = file_to_weird(ori)
    tgt_list = file_to_weird(tgt)
    pred_list = file_to_weird(pred)
    ori_pred_dis = compute_ted(src_list, pred_list)
    tgt_pred_dis = compute_ted(tgt_list, pred_list)
    print("Sim To Ori", sum(ori_pred_dis) / len(ori_pred_dis))
    print("Sim To Tgt", sum(tgt_pred_dis) / len(tgt_pred_dis))


def evaluate_pairwise_ted(tree_list):
    ted_scores = []
    for i in range(tree_list):
        for j in range(i, len(tree_list)):
            ted_scores.append(compute_ted(tree_list[i], tree_list[j]))

    return (sum(ted_scores) * 2.0) / (len(tree_list) - 1) / len(tree_list)
